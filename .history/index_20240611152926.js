...
(async () => {
    ...
    try {
        ...
        const phones = $('._1AtVbE'); // Update selector to match phone containers

        phones.each((_, elem) => {
            const container = $(elem);
            const title = container.find('._4rR01T').text() || container.find('a.s1Q9rs').text(); // Title selector
            const price = container.find('._30jeq3').text(); // Price selector
            const rating = container.find('._3LWZlK').text(); // Rating selector
            const numberOfRatings = container.find('._2_R_DZ span').first().text(); // Number of ratings selector
