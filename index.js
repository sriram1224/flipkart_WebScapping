    ...
    while (retries > 0) {
        try {
            ...
            const data = await page.evaluate(() => {
                ...
                phones.forEach((elem) => {
                    const title = elem.querySelector('.KzDlHZ')?.textContent; // Title selector
                    const price = elem.querySelector('.Nx9bqj._4b5DiR')?.textContent ; // Price selector
                    const rating = elem.querySelector('.XQDdHH')?.textContent ; // Rating selector
                    const numberOfRatings = elem.querySelector('Wphh3N span')?.textContent ; // Number of ratings selector
